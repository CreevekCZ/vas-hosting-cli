...
public struct FTPUnlockCommand: AsyncParsableCommand {
    public static let configuration = CommandConfiguration(
        commandName: "unlock",
        abstract: "Unlock (re-enable) an FTP account."
    )
    ...
    public func run() async throws {
        ...
        let response = try await client.unlockFtpAccount(path: .init(domain: domain, ftp: ftp))
        switch response {
        case .noContent:
            OutputFormatter.printSuccess("FTP account '\(ftp)' unlocked.", format: clientOptions.format)
